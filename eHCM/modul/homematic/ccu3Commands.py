...
class ccu3Commands(defaultModul):
    '''
    classdocs
    '''
    ...
    def __formatOutput(self,deviceRAW):
        """
        format the ccu3 output in eHMC format
        ...
        try:
            deviceID=""
            for device in deviceRAW:
                if device["PARENT"]:
                    """
                    find channel in RAW data
                    """
                    channelName=device['ADDRESS']
                    self.device[deviceID]['channels'][channelName]={"parameter":{"channelPackage":MODUL_PACKAGE}}
                    for attribute in device:
                        if attribute in CHANNEL_IGNORE_FIELD:
                            """ find attribute in CHANNEL_IGNORE_FIELD to ignore"""
                            continue 
                        
                        eHMCattribut="unkown"
                        """ format attribute Mapping"""
                        if attribute in CHANNEL_MAPPING_FIELDS:
                            eHMCattribut=CHANNEL_MAPPING_FIELDS[attribute]
                        else:
                            eHMCattribut=attribute
                        
                        self.device[deviceID]['channels'][channelName]['parameter'][eHMCattribut]=device[attribute]
                else:
                    """
                    find device in RAW data
                    """
                    deviceID=device['ADDRESS']
                    self.device[deviceID]={"channels":{},
                                           "parameter":{"devicePackage":MODUL_PACKAGE},
                                           'enable':True,
                                           }
                    for attribute in device:
                        if attribute in DEVICE_IGNORE_FIELD:
                            """ find attribute in DEVICE_IGNORE_FIELD to ignore"""
                            continue 
                        ...
                        """deviceType parameter replace "-" to "_" """
                        attributValue="unkown"
                        if eHMCattribut=="deviceType":
                            attributValue=device[attribute].replace("-","_")
                        else:
                            attributValue=device[attribute]
                            
                        self.device[deviceID]['parameter'][eHMCattribut]=attributValue
            return self.device
        except:
            raise defaultEXC("unkon error in formatOutput",True)   
